...
## F15
Severity: LOCAL
Status: defect (adjudicated 2026-09-15). Confirmed: `claimLock` (`src/store/index.ts` L944–1003) waits one 10 s observation (L39, L973) and refuses `storage_locked` whenever a live holder renewed (L986–988), so a 12 s retry allows about one attempt against a worker that is still draining; the target's own H9 L171 anticipated this. Nuances: §10 L640 documents disabling overlap as an alternative, so the failure requires overlap enabled with consoles attached (the IIS defaults are [X], unverified here); A13 L354 says "may", so the defect is that the only concrete policy offered cannot cover the scenario A13 names. Unverified: the drain length depends on when ASP.NET Core calls the hosted service's `StopAsync` (L634) relative to draining long-lived requests; if the runtime stops at `ApplicationStopping`, `releaseLock` removes the file and the new worker claims promptly. v2 replaces the fixed retry with an unavailable/retrying state and opt-in startup timeout (A13 L380–381, §10 L659–660, §15 L867).
...
## F17
Severity: LOCAL
Status: defect (adjudicated 2026-09-15), narrowed. Sustained: A17 puts secret resolution in the v1 protocol (L363–364, Phase 4 L783) that SkyNetHR migrates onto at step 6 (L786–788), and the target never states that SkyNetHR stays a CLI-only consumer that does not declare it. Not sustained: runtime-held conversation state, since only the `conversationState` declaration is v1 (L653, L667) and §11 L685 lists the history store as a "Missing capability" of API providers arriving in Phase 10 (L802). Nuances: A17 L365 gates callbacks on methods the client declares in `runtime.hello`, so the path is dormant surface rather than a held credential; the H1 consequence conflates env-inherited `ANTHROPIC_API_KEY` (already addressed by A9 L332) with configuring an API provider that does not exist until Phase 10. Not a brief conflict: nothing requires the brief (L75, L78–79) to change. v2 draws the boundary (L396, L690, L707–708, L828, L872).
