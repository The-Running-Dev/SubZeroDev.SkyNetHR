...
I50 requires of either mechanism and the only thing the two have in common. They share no
evidence and are deliberately unlike, because `server.lock` must hold across a container and
another host (D180) where this one need only hold across processes that can see each other's
`process.kill(pid, 0)`. The host claims both, `server.lock` first (*Public surface § `store`*),
