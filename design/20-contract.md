...
| `runtime-leases/<instanceId>.json` | `instanceId`, minted per run | — | Server-wide, and the **second** mechanism holding I50's one-process-per-root property — `server.lock` is the host's, this is the runtime's, and a host claims both. One file per contender, published whole by exclusive-create-then-`link` so a generation is never overwritten and never read torn. A contender publishes before it enumerates, so two can both refuse but cannot both see themselves alone; **there is no observation window and no retry delay**, which is what distinguishes this from `server.lock`'s reclaim — and D262 leaves that standing, because its one new ground borrows an observation `server.lock` already completed rather than making one. A holder carries `serverLockInstanceId`: the `server.lock` `instanceId` its host held when it claimed, or `null` for a runtime with no host. It is disregarded on exactly two grounds (D262). **Proven local death** — same hostname, and `ESRCH` or an OS creation time that does not match. **A displaced generation** — the claimant itself holds `server.lock`, and the holder names a *different* `serverLockInstanceId`, on any hostname. **Every other holder fails closed**: a `null` holder on another hostname, a holder whose liveness cannot be established, and every foreign-hostname holder when the claimant holds no `server.lock`. A present `serverLockInstanceId` that is neither a string nor `null` is `corrupt`. Not durable: a lease lost to a host crash is a dead holder, which is the same answer. Removed on release |
...
| `runtime-leases/<instanceId>.json` | a lease left by a crash or a torn-down container, written before D262 | Written whole once per claim and removed on release, so only a lease nobody released outlives the build that wrote it. **A holder written before D262 carries no `serverLockInstanceId`, and an absent field is not `null`** — `null` says the runtime had no host, absent says nothing either way. Where an absent-field holder names another hostname and the claimant holds `server.lock`, it reads as a displaced generation, which is what lets a volume already stranded by #403 boot with no operator action: D196's precedent, a missing field reaching the reclaim path rather than a refusal that strands the deployment the migration exists for. On the claimant's own hostname an absent field changes nothing, and proven local death decides as before. **The residual is named rather than buried**: a pre-D262 standalone runtime still live in another container over the same volume at the upgrade is disregarded. It ends with the last pre-D262 lease file |
...
- **`serverLockInstanceId` is optional on the type because it is optional on disk, and absent is
  ...
  `null`. Absent means written before D262, and the two decide differently on another hostname
  (*Persisted schemas § Migration*).
- **`heldServerLock` is required, and must never acquire a default.** A default of `() => null`
  would compile every host call site that forgot it into the standalone rule, reinstating #403
  without a type error. The host passes an accessor for the `server.lock` `instanceId` it holds
  (`null` once released). The standalone runtime (`src/agent-console/runtime/server.ts`) passes
  `() => null` at its own call site, where the absence of a host is visible.
- **It is read at claim time, not at construction.** `createStore` builds the session store
  before `boot` mints and claims the lock, so a value captured at construction is always `null`.
  The claim both stamps the accessor's current value into this contender's own holder and
  compares it against every other holder. A host whose accessor returns `null` at claim time
  holds no lock to lend, and gets the standalone rule, which is the safe direction.
...
a bridge, not a guard the runtime enforces** (I65). Nothing in `server.ts` consults that table;
...
**The runtime holds a storage root under `runtime-leases/`, never `server.lock`, and I50 is one
invariant over both.** `SessionCore.boot()` claims it before the reap step, which is the ordering
I50 requires of either mechanism and the one thing every runtime shares with `server.lock`. The
two are deliberately unlike, because `server.lock` must hold across a container and another
host (D180) where this one decides liveness itself only across processes that can see each
other's `process.kill(pid, 0)`. **Past that reach it borrows `server.lock`'s evidence rather than
growing its own** (D262): a host-claimed lease names the `server.lock` generation it was taken
under, and a claimant holding a different generation disregards it on any hostname, because
D180's observation and D216's confirmation have already established that the earlier
generation's server lost the root. That is how a lease left by a recreated container stops
refusing every later boot (#403). A runtime with no host has no generation to lend and keeps the
fail-closed rule. The host claims both, `server.lock` first (*Public surface § `store`*),
so a runtime pointed at a live SkyNetHR root is refused at the lease it does share. **What must
never be built on this is a second holder of either kind reaching the reap step**, for the reason
I50 gives: I19's guard cannot tell which lease recorded a pid.
...
rather than given a sibling** (D227's precedent), which is this section's decision rather than
I50's; what it costs — a `renewals` that is a placeholder and not a counter anyone may watch — is
I50's, stated there once. `image: 'runtime'` is the discriminator, and it is informational like
every other field on that type (I57).
...
**An A5 create attempt whose outcome is unknown is quarantined, not rolled back** (I66). Where
...
- **`boot` has exactly one counterpart, and it exists because shutdown's step 3 has nowhere
  ...
  **The mute is at the sink and not in the `exited` handler, and the tree is what settles
  ...
  the three the outcome turns on**: a `spawned` arriving behind the mute is not simply dropped.
  Its branch warns, then kills the pid the notification itself carries —
  `killProcessTree(n.pid, n.pgid)`, the reap-kill D23's guard already uses — because a
  `message()` that had claimed the turn slot before the kill pass ran can still spawn its child
  afterward, past the point step 1 could have reached it. That child is never recorded in
  `pids.ndjson` and no tombstone is owed for it: it is killed before it executes an instruction
  of its own, so no marker of its own existence is reachable, and the warn line is the only
  witness (D198).
...
| `storage_locked` | The runtime lease is held by a live process. `data.detail` names the holder and its age | **Only on evidence**, never on a timer | Nothing automatic. There is no fixed retry delay and no observation window; a caller that polls is guessing where `server.lock`'s half of I50 would have watched a counter |
...
| **I27** | Caller serialization is limited to Phase 3b's short-lived per-session lanes for `seq` assignment and atomic state transitions, and a short-lived global workspace-allocation guard for decisions over overlapping live, pending and exclusive reservations (D238, A11/A22). Each guarded section is synchronous and releases before I/O, provider execution, permission waits, host callbacks or any other long-running await; the turn/busy state and reservations it establishes may outlive it and are not held guards. Same-tick check-and-claim exclusivity (I5) and existing event ordering (including I64) are unchanged: `emit`'s synchronous prefix — `seq`, ring push, fan-out — remains the event serialisation point. The per-session append chain that follows it orders I/O and excludes nothing (D89); no lane or allocation guard is held while awaiting it. No general-purpose caller mutex, lock or semaphore is authorized beyond these two mechanisms. `server.lock` remains a separate process-level lease excluding a second *process*, not a second caller (D161, D180); its claim, renewal and release rules are unchanged | `session-manager` / Phase 3b core; all callers |
...
| **I50** | A storage root is held by at most one process, and **whichever mechanism holds it, the claim completes before boot's reap step** — not merely before `listen`. That ordering is what the reap step relies on, whichever mechanism holds: I19's reap guard tests hostname, image and creation time, never which lease recorded the pid, so a process reaping a root it does not hold would kill another holder's live children and pass every check doing it. **A server claims `<storage>/server.lock`**, reclaimed only where its `(instanceId, renewals)` pair is observed unchanged across one full observation window measured on the claiming process's own monotonic clock — **whichever host wrote it**, and with no wall clock compared anywhere in the decision (D180). **A reclaim is confirmed before boot work**: the reclaimer re-samples one renewal interval after its overwrite and proceeds only on its own `instanceId` (D216). A boot that refuses on a held lock has written nothing server-wide. **A storage root is supported on a local filesystem or a bind mount only** (D194): the one hole — a live holder whose renewals are invisible for a full window — needs a cache or a partition between writer and reader, which two processes on one host do not have, so it is out of reach rather than tolerated. It is what a network-share root would reintroduce, and closing it there would need a fencing service outside the storage root (D7). **An AgentConsole runtime claims `runtime-leases/<uuid>.json` instead**, publishing its claim and then enumerating the directory, so two contenders can both refuse but **cannot both see themselves alone**; liveness is decided from process existence on this host plus a matching OS creation time, and **a holder claimed under a `server.lock` generation other than the one the claimant now holds is disregarded on any hostname** (D262) — the one point where the runtime half reads the server half's evidence, only in that direction, and only by a claimant that holds `server.lock`; every other holder on another hostname, and one whose liveness cannot be established, **fails closed**. **That half has no observation window, no renewal counter and no fixed retry delay**, so a `storage_locked` raised by it is not the reclaimable lock the first half describes and **nothing may wait for evidence it never produces** — including the `renewals: 0` synthesised on a runtime holder reported through `StartupError.storage_locked`, which is a placeholder and not a counter (I57). **Neither mechanism excludes the other's file, and nothing needs it to**: the host claims both, `server.lock` first, which is what keeps a runtime and a server off one root | `store`, `session-manager` |
