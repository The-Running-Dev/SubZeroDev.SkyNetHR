...
- **A dragged ticket has no defined effect, and operator-driven assignment has no
  ...
  a session sends it as a message or labels the session alongside one; those are different
  features and the queue has no home in storage either way. D59's tier two is items 8 to 12 and
  none of them is a backlog, so the same gap now has a second face: either assignment is
  permitted but not required — which is what `10-design.md` designs against, and why nothing
  provides for it — or the definition of done is one item short. Both halves are the same owner
  question. See `10-design.md § Open questions` item 13.
...
- **The two-platform gate D64 requires has no slice.** `/slices` derives from the contract, and
  ...
  cannot close without it. `10-design.md § Failure modes § Platform divergence` now enumerates
  the six surfaces that gate would have to cover, so the target list exists even though the
  gate does not.
- **The token budget's scope is undecided.** D53 calls a budget "a value the operator sets in
...
- **Whether a vendor's `usage` numbers are cumulative or incremental is unverified.** D75 puts
  ...
  correct rather than plausible. Cheap to test on Claude; blocked behind S8.1 for Codex. See
  `10-design.md § Open questions` item 14.
