...
export function createCheckpoints(config: Pick<RuntimeOptions, 'storageRoot'>, identity: { name: string; email: string }): Checkpoints {
  return {
    async init(sessionId: SessionId, cwd: ResolvedPath) {
      ...
      // repositories.
      const name = await runGit(gitDir, cwd, ['config', 'user.name', identity.name]);
      if (!name.ok) return { ok: false, error: initFailure(name.error) };
      const email = await runGit(gitDir, cwd, ['config', 'user.email', identity.email]);
